...
describe("POST /employees", () => {
    ...
    afterAll(async () => {
        const _ =  await request(BASE_URL).delete(`/employees/${addedEmployee?.empID}/delete`).send().set({"Authorization": `Bearer ${accessToken}`});
    });
    test("Test Can update employee.", async () => {
        expect(addedEmployee?.fullName).toBe("Kenneth Silas");
        let newEmployee = employee;
        newEmployee.fullName = "Janet Kimani";
        let result1 = await request(BASE_URL).put(`/employees/${addedEmployee?.empID}`).send(newEmployee).set({"Authorization": `Bearer ${accessToken}`});
