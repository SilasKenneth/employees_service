...
describe("GET /employees/(|{emp_id})", async () => {
    ...
    afterAll(async () => {
        await request(BASE_URL).delete(`/employees/${emp?.empID}/delete`).send().set({"Authorization": `Bearer ${accessToken}`});
    });

    test("Can get all employees", async () => {
        let result2 = await request(BASE_URL).get(`/employees/${emp?.empID}`).send().set({"Authorization": `Bearer ${accessToken}`});
